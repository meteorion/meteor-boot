...
import pers.meteor.security.core.exception.OAuth2ErrorCodesExpand;
import pers.meteor.security.core.exception.OAuthClientException;
import pers.meteor.security.core.util.OAuth2EndpointUtils;
...
public class TokenEndpoint {
	...
	public ModelAndView confirm(Principal principal, ModelAndView modelAndView,
		...
		OauthClientDetailsDTO clientDetails = RetOps.of(clientDetailsService.getClientDetailsById(clientId))
			.getData()
	 ...
	 * 令牌列表
	 * @param params 参数
	 * @return {@link R }<{@link PageR<OAuthToken> }>
	 */
	@Inner
	@PostMapping("/token/page")
	public PageR<OAuthToken> tokenList(@RequestBody Map<String, Object> params) {
		// 根据分页参数获取对应数据
		...
		List<OAuthToken> tokenVoList = redisTemplate.opsForValue().multiGet(pages).stream().map(obj -> {
			...
			OAuthToken tokenVo = new OAuthToken();
